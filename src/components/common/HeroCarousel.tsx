...
function HeroCarousel() {
  ...
  const prev = () => setActive(s => (s + 2) % 3)
  const next = () => setActive(s => (s + 1) % 3)
  ...
  return (
    <div style={{ position: 'absolute', left: 0, top: 817, width: 1920, height: 960, overflow: 'hidden' }}>
      {/* Slides — zoom + fade */}
      {SLIDES.map((s, i) => (
        <div key={i} style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: i === active ? 1 : 0, transition: 'opacity 1s ease' }}>
          <img
            key={`${i}-${active}`}
            src={s.img} alt=""
            style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transformOrigin: 'center', animation: i === active ? 'cs-zoom 6s ease-in-out forwards' : 'none' }}
          />
        </div>
      ))}
      ...
      {/* Text content */}
      <div style={{ position: 'absolute', inset: 0, zIndex: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, padding: '0 220px' }}>
        <p style={{ margin: 0, fontFamily: 'Inter', fontSize: 26, letterSpacing: '8px', color: '#f2a8c4', textTransform: 'uppercase' }}>
          {slide.label}
        </p>
        <div style={{ width: 280, height: 1.5, background: 'linear-gradient(to right, transparent, rgba(201,169,110,0.9), transparent)' }} />
        <h2 style={{ margin: 0, textAlign: 'center', fontFamily: 'Lora', fontStyle: 'italic', fontWeight: 400, fontSize: 108, lineHeight: 1.18, color: '#fff', textShadow: '0 3px 28px rgba(0,0,0,0.45)' }}>
          {slide.title}
        </h2>
        <p style={{ margin: 0, textAlign: 'center', fontFamily: 'Alice', fontSize: 36, lineHeight: 1.55, color: 'rgba(255,255,255,0.87)' }}>
          {slide.sub}
        </p>
